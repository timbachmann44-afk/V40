...
# CONFIG
# =========================
st.set_page_config(page_title="V54 INSTITUTIONAL AI", layout="wide")

st.title("🏆 V54 INSTITUTIONAL AI (ALL COINS MODE)")
...
coins = [
    ...
    "SOL/USD","ADA/USD","DOGE/USD","BNB/USD"
]
...
def engine(price, support, resistance, atr, momentum, zone):
    ...
    if zone == "LONG_ZONE":
        ...
        tp = resistance
        score += 30
        reasons.append("Institutional LONG zone")
        ...
        tp = support
        score += 30
        reasons.append("Institutional SHORT zone")
...
# =========================
# FILTER SAFE TRADE SET
# =========================
df_valid = df[df["Signal"] != "NO TRADE"]

if df_valid.empty:
    st.warning("No institutional setup found")
    st.stop()
...
c1.metric("📊 COINS SCANNED", len(df))
c2.metric("🎯 ACTIVE SETUPS", len(df_valid))
c3.metric("🔥 BEST SCORE", best["Score"])
